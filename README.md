...
- **Install Jetson Nano base image**: install its base image, [JetPack version 4.2](https://developer.nvidia.com/embedded/learn/get-started-jetson-nano-devkit). It is based on Ubuntu 18.04 and already includes NVIDIA drivers version > 418, CUDA and Nvidia-Docker.

> [!WARNING]
> This repo only works with Jetpack 4.2 for now. NVIDIA released Jetpack 4.3 recently and an update to DeepStream container in the marketplace to support Jetpack 4.3 is on its way. For now please use a Jetpack 4.2.
